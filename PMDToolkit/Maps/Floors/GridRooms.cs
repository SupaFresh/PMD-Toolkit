...
namespace PMDToolkit.Maps.Floors
{
    public class GridRooms : RandomMap
    {
        ...
        public override void Generate(int seed, RDungeonFloor entry, List<FloorBorder> floorBorders, Dictionary<int, List<int>> borderLinks)
        {
            ...
            do
            {
                if (rand.Next(0, (2 + pathsMade)) == 0)
                {//will end the current path and start a new one from the start
                    if (rand.Next(0, 2) == 0)
                    {//determine if the room should be open or a hall
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        if (nextDir != prevDir)
                        {//makes sure there is no backtracking
            ...
            isDone = false;
            do
            { //determine ending room randomly
            ...
            isDone = false;
            do
            { //determine starting room randomly
                int x = rand.Next(0, Rooms.GetLength(0));
                int y = rand.Next(0, Rooms.GetLength(1));
                if (Rooms[x, y].Opened == DungeonArrayRoom.RoomState.Open)
                {
        ...
        private void addSEpos(Loc2D point, bool isStart)
        {
            ...
            while (true)
            {//backup plan in case rooms are so small that all there's left is ","
                randx = rand.Next(x, u + 1);
                randy = rand.Next(y, v + 1);
        ...
        private void createHHall(int hallX, int hallY)
        {
            ...
            {//checks if at least one turn is needed to make rooms meet
                m = 1;
            }
        ...
        private void DrawHalls(DungeonArrayHall hall)
        {
        ...
        private void DrawHall(Loc2D point1, Loc2D point2, ref bool addedEntrance)
        {
        ...
        private void DrawHallTile(int x, int y, ref bool addedEntrance)
        {
